...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    UIView *view = nil;
		...
		if ([self isRowSelectedInTable:tableView withRow:indexPath.row]){
			//NSLog(@"row is %d and it is selected", indexPath.row);
			view = [self viewForComponent:component inRect:viewRect isHighlighted:YES];
		}else{
			//NSLog(@"row is %d and it is not selected", indexPath.row);
			view = [self viewForComponent:component inRect:viewRect isHighlighted:NO];
		}
...
- (BOOL)isRowSelectedInTable:(UITableView*)table withRow:(int)row{
	if(table.tag == 0){
		if ([[self.selectedRowIndexes objectAtIndex:0] isEqual:[NSNumber numberWithInt:row]]){
			return true;
		}
	}else{
		if ([[self.selectedRowIndexes objectAtIndex:1] isEqual:[NSNumber numberWithInt:row]]){
			return true;
		}
	}
	return false;
...
- (UIView *) viewForComponent:(NSInteger)component inRect:(CGRect)rect isHighlighted:(BOOL)highlighted
{
    ...
    switch (component) {
        case 0:
			label.textColor = highlighted? [UIColor blackColor] : [UIColor blackColor];
...
- (void)setDate:(NSDate *)date animated:(BOOL)animated{
    ...
    [self selectRow:minute inComponent:1 animated:YES];
...
- (NSString*)getPeriodOfDay{
	int hour = [self selectedRowInComponent:0]%24;
	if (hour >= 0 && hour <= 12){
...
- (NSString*)getTimeFromDate{
	...
	if (![[formattedDateString substringToIndex:1] isEqualToString:@"0"]){
		return formattedDateString;
	}else{
		return [formattedDateString substringWithRange:NSMakeRange(1, formattedDateString.length-1)];
	}
