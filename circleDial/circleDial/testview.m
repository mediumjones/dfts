...
- (void)setUpTimerWithCountDownTimer:(int)minutes withTimerLabel:(UILabel*)timerLabel withWrittenTimerLabel:(UILabel*)writtenTimerLabel{
	...
	writtenTimerLabel.text = [self getWrittenTimeFromMinutes:90];
	timerLabel.text = [self timeFormatted:90];
}
...
- (void)drawRect:(CGRect)rect
{
	...
	if (self.bounds.size.width > self.bounds.size.height){
		...
		radius = self.bounds.size.width/2 * 0.88;
	}
	    ...
	    if (self.progress != 0) {
	       ...
	       [progressCircle stroke];
	        ...
	        UIBezierPath *progressCircle = [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.bounds.size.width / 2,self.bounds.size.height / 2)
	                                                                      radius:radius - self.lineWidth / 2                                                                  startAngle:(CGFloat) 3 * M_PI/2 + M_PI/9
...
- (NSString*)returnWrittenTime{
	if (self.progress == 0) return @"one minute";
	else
	return [self getWrittenTimeFromMinutes:self.progress * self.secondsTotal];
}
@end
